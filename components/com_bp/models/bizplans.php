...
class BpModelBizplans extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('categories_2665151.title AS category2');            
		$query->join('LEFT', '#__categories AS categories_2665151 ON categories_2665151.id = a.category');
		...
		//$query->where('a.lang = ' . $db->Quote($lang->getTag()));
...
if ($id_bizplansforbizplantexts > null) {
		$query->where('a.id = ' . (int) $id_bizplansforbizplantexts);
	}
...
if ($tovarfilteredbyuser > null) {
		$query->where('a.id = ' . (int) 0);
            ...
            if (!empty($search))
            {
				...
				$query->where('( a.name LIKE ' . $search . '  OR categories_2665151.title LIKE ' . $search . '  OR  a.description LIKE ' . $search . '  OR  a.address LIKE ' . $search . ' )');
                }
            }
